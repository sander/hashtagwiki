...
fn pages_containing(hashtag: &HashTag) -> Result<Vec<(document::PageId, document::PageTitle)>, io::Error> {
    ...
    dir.map(|entry| {
        ...
        if document::transform(content, id.clone()).1.contains(&hashtag) {
...
pub async fn serve(address: impl Into<SocketAddr>) {
    let wiki = warp::path!("wiki" / String).map(|name| {
        let id = document::PageId(String::from(name));
        let path = Path::new("wiki/name")
            .with_file_name(id.0.clone())
            .with_extension("md");
    ...
    let hashtag = warp::path!("hashtag" / String).and_then(get_hashtag_info);
